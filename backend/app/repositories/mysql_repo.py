...
class QuestionRepository:
    ...
    def get_sessions_by_user(self, user_id: Optional[int] = None) -> List[dict]:
        """Lấy danh sách sessions duy nhất, kèm câu hỏi đầu tiên làm tiêu đề."""
        from sqlalchemy import distinct
        query = self.db.query(
            models.Question.session_id,
            func.min(models.Question.created_at).label("created_at"),
            func.count(models.Question.id).label("total_questions"),
        ).group_by(models.Question.session_id)

        if user_id is not None:
            query = query.filter(models.Question.user_id == user_id)

        rows = query.order_by(func.min(models.Question.created_at).desc()).limit(50).all()

        sessions = []
        for row in rows:
            # Lấy câu hỏi đầu tiên của session làm tiêu đề
            first_q = (
                self.db.query(models.Question)
                .filter(models.Question.session_id == row.session_id)
                .order_by(models.Question.created_at)
                .first()
            )
            sessions.append({
                "session_id": row.session_id,
                "title": first_q.noi_dung_cau_hoi[:60] + ("…" if len(first_q.noi_dung_cau_hoi) > 60 else "") if first_q else "Phiên không có tiêu đề",
